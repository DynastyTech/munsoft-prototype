      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
          >
            <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-gray-800 p-2 shadow-lg transform transition-transform duration-300 group-hover:rotate-12">
              <img
                src={logoITNA}
                alt="ITNA Logo"
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <motion.a
                key={item.name}
                href={item.href}
                className="text-gray-300 hover:text-blue-400 font-medium transition-colors relative group"
                whileHover={{ y: -2 }}
              >
                {item.name}
